...
void AudioCodec::decompressAudioLossless()
{
...
void AudioCodec::compressAudioLossy(float lb)
{
    ...
    prev_res0_l = left_channel[0] * lb;
    code=golombEncoder.encodeNumber(prev_res0_l*32768);
    writeSample(bs, code);
    prev_res0_r = right_channel[0] * lb;
    code=golombEncoder.encodeNumber(prev_res0_r*32768);
    writeSample(bs, code);
    prev_res1_l = left_channel[1] * lb;
    code=golombEncoder.encodeNumber(prev_res1_l*32768);
    writeSample(bs, code);
    prev_res1_r = right_channel[1] * lb;
    code=golombEncoder.encodeNumber(prev_res1_r*32768);
    writeSample(bs, code);
    prev_res2_l = left_channel[2] * lb;
    code=golombEncoder.encodeNumber(prev_res2_l*32768);
    writeSample(bs, code);
    prev_res2_r = right_channel[2] * lb ;
    code=golombEncoder.encodeNumber(prev_res2_r*32768);
    writeSample(bs, code);
    ...
    for(int nsample=3; nsample<sourceAudio.getNumSamplesPerChannel(); nsample++)
    {
        ...
        // calculo dos residuais direitos
        res0_r = right_channel[nsample] * lb;
        res1_r = res0_r - prev_res0_r;
        res2_r = res1_r - prev_res1_r;
        res3_r = res2_r - prev_res2_r;
        ...
        // calculo dos residuais esquerdos
        res0_l = left_channel[nsample] * lb;
        res1_l = res0_l - prev_res0_l;
        res2_l = res1_l - prev_res1_l;
        res3_l = res2_l - prev_res2_l;
...
void AudioCodec::decompressAudioLossy(float lb)
{
    std::cout << "\n ########## Starting Decoding Process ########## " << std::endl;
    Golomb golombDecoder = Golomb(this->m);
    BitStream bs = BitStream(this->residualFileName,'r');
    ...
    while (bitsRead < (bitsInFile-11)) 
    {
        ...
        if(firstSamplesFlag)
        {
            code = decodeCode(bs, restBits, bitsRead); 
            left_channel_sample = golombDecoder.decodeNumber(code);
            prev_val0_l = (float)(left_channel_sample)/lb;
            outFile.samples[0][0] = prev_val0_l/32768;
            
            code = decodeCode(bs, restBits, bitsRead); 
            right_channel_sample = golombDecoder.decodeNumber(code);
            prev_val0_r = (float)(right_channel_sample)/lb;
            outFile.samples[1][0] = prev_val0_r/32768;

        //////////////////////////////////////////////////////////////
            code = decodeCode(bs, restBits, bitsRead); 
            left_channel_sample = golombDecoder.decodeNumber(code);
            prev_val1_l = (float)(left_channel_sample)/lb;
            outFile.samples[0][1] = prev_val1_l/32768;
            
            code = decodeCode(bs, restBits, bitsRead); 
            right_channel_sample = golombDecoder.decodeNumber(code);
            prev_val1_r = (float)(right_channel_sample)/lb;
            outFile.samples[1][1] = prev_val1_r/32768;

    //////////////////////////////////////////////////////////////////
            code = decodeCode(bs, restBits, bitsRead); 
            left_channel_sample = golombDecoder.decodeNumber(code);
            prev_val2_l = (float)(left_channel_sample)/lb;
            outFile.samples[0][2] = prev_val2_l/32768;
            
            code = decodeCode(bs, restBits, bitsRead); 
            right_channel_sample = golombDecoder.decodeNumber(code);
            prev_val2_r = (float)(right_channel_sample)/lb;
            outFile.samples[1][2] = prev_val2_r/32768;
